...
pub enum Type {
    ...
    Void,       // For functions
    #[default]
    Unknown, // For untyped variables (to be inferred)
}
...
fn type_annotate_expression<Annotation: ASTAnnotation>(
    ...
    match ast_expr {
        ...
        Expression::FnCall {
            ...
            let function_signature = function_signature_map
                .get(&function_id)
            ...
            for (i, typed_arg) in typed_arguments.iter().enumerate() {
                let expected_type = *function_signature
                    .param_types
                    .get(i)
                ...
                assert_eq!(
                    typed_arg.get_type(),
                    expected_type,
                    "Type mismatch in argument {} of function call to '{}'",
                    i,
                    name
                );
            }
